...
const LastLearningSessionsList = ({data}: { data: LearnedTime[] }) => {
    return (
        <div className="flex flex-col gap-3 items-center  shadow-md shadow-sky-100 rounded-md p-5  overflow-auto">
            ...
            <List width={420} height={500}
                  rowCount={data.length}
                  style={{overflow:"scroll",padding:"5px"}}
                  rowHeight={50}
                  rowRenderer={({key, index}) => {
                      const i = data[index];
                      const duration = moment.duration(moment(i.endDate).diff(i.startDate));
                      return <div key={i.id} className="flex mt-2 rounded-md justify-between p-2 bg-blue-100">
                          <div>{moment(i.startDate).format("hh:mm:ss")} - {moment(i.endDate).format("hh:mm:ss")}</div>

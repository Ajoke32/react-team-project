...
const TopicsColumnList = () => {
    ...
    return (
        <div className="flex flex-col gap-2 items-center  shadow-md shadow-sky-100 rounded-md p-5">
            ...
            {topics.map(t=>{
                return <div key={t.id} className="flex mt-2 cursor-pointer rounded-md justify-between p-2 bg-purple-100 w-96">
                    <div><a href={`topics/${t.id}`}>{t.title}</a></div>
                    <div>&rarr;</div>
                </div>
            })}
        </div>
